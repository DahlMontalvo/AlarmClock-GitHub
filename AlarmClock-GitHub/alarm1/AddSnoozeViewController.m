...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ((indexPath.section == 0 && [[[Singleton sharedSingleton] sharedPrefs] integerForKey:@"newAlarmSnoozeInterval"] == [[[snoozeSettingsValues objectAtIndex:indexPath.section] objectAtIndex:indexPath.row] intValue]
        ) ||
        (indexPath.section == 1 && [[[Singleton sharedSingleton] sharedPrefs] integerForKey:@"newAlarmSnoozeNumberOfTimes"] == [[[snoozeSettingsValues objectAtIndex:indexPath.section] objectAtIndex:indexPath.row] intValue])) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (int i = 0; i < [[snoozeSettings objectAtIndex:0] count]; i++) {
        if ([[tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]] accessoryType] == UITableViewCellAccessoryCheckmark) {
            [[[Singleton sharedSingleton] sharedPrefs] setInteger:[[[snoozeSettingsValues objectAtIndex:0] objectAtIndex:i] intValue] forKey:@"newAlarmSnoozeInterval"];
        }
    }
    for (int i = 0; i < [[snoozeSettings objectAtIndex:1] count]; i++) {
        if ([[tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:1]] accessoryType] == UITableViewCellAccessoryCheckmark) {
            [[[Singleton sharedSingleton] sharedPrefs] setInteger:[[[snoozeSettingsValues objectAtIndex:1] objectAtIndex:i] intValue] forKey:@"newAlarmSnoozeNumberOfTimes"];
        }
    }
}
